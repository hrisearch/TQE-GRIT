...
def get_query_df(qset):
    ...
    query_df = df_test.filter(['query_id'], axis=1)
    ...
    task_query_df = pd.read_csv('TQE.csv')
    task_query_df['qid'] = task_query_df['qid'].astype(str)

    query_df = pd.merge(query_df, task_query_df, on='qid', how='inner')
    query_df = query_df.filter(['qid', 'query'], axis=1)
...
for qset in ['large']:
    ...
    spres.to_pickle('retrieval_results_task2/splade_' + qset + '.p')
